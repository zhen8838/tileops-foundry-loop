...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(const half_t* __restrict__ activation, half_t* __restrict__ output, const uchar* __restrict__ packed_weight, const float* __restrict__ weight_scale, const uchar* __restrict__ weight_zero) {
  ...
  float scales[1];
  float zeros[1];
  uchar packed_weight_local_cast[4];
  int packed_values[4];
  float products_low[4];
  float products_high[4];
  float partial_low[1];
  float partial_high[1];
  accum[0] = 0x0p+0f/*0.000000e+00*/;
  activation_shared[((int)threadIdx.x)] = activation[((((int)blockIdx.y) * 128) + ((int)threadIdx.x))];
  scales[0] = weight_scale[((((int)blockIdx.x) * 8) + (((int)threadIdx.x) >> 4))];
  zeros[0] = ((float)weight_zero[((((int)blockIdx.x) * 8) + (((int)threadIdx.x) >> 4))]);
  *(uint*)(packed_weight_local_cast + 0) = *(uint*)(packed_weight + ((((int)blockIdx.x) * 512) + (((int)threadIdx.x) * 4)));
  int4 __1;
  uint v_ = *(uint*)(packed_weight_local_cast + 0);
  __1.x = (int)(((unsigned char)(v_ >> 0)));
  __1.y = (int)(((unsigned char)(v_ >> 8)));
  __1.z = (int)(((unsigned char)(v_ >> 16)));
  __1.w = (int)(((unsigned char)(v_ >> 24)));
  *(int4*)(packed_values + 0) = __1;
  __syncthreads();
  #pragma unroll
  for (int i = 0; i < 4; ++i) {
    int low = (packed_values[i] & 15);
    half_t dequant_low = ((half_t)((((float)low) - zeros[0]) * scales[0]));
    products_low[i] = (((float)activation_shared[(((((int)threadIdx.x) & 15) * 8) + (i * 2))]) * ((float)dequant_low));
  }
  #pragma unroll
  for (int i_1 = 0; i_1 < 4; ++i_1) {
    int high = (packed_values[i_1] >> 4);
    half_t dequant_high = ((half_t)((((float)high) - zeros[0]) * scales[0]));
    products_high[i_1] = (((float)activation_shared[((((((int)threadIdx.x) & 15) * 8) + (i_1 * 2)) + 1)]) * ((float)dequant_high));
  }
  ...
  partial_low[0] = tl::AllReduce<tl::SumOp, 16, 1, 0, tl::NamedBarrier<128>>::run(partial_low[0]);
  partial_high[0] = 0x0p+0f/*0.000000e+00*/;
  #pragma unroll
  for (int rv_1 = 0; rv_1 < 4; ++rv_1) {
    partial_high[0] = (partial_high[0] + products_high[rv_1]);
  }
  partial_high[0] = tl::AllReduce<tl::SumOp, 16, 1, 0, tl::NamedBarrier<128>>::run(partial_high[0]);
  accum[0] = ((accum[0] + partial_low[0]) + partial_high[0]);
  if ((((int)threadIdx.x) % 16) == 0) {
    output[(((((int)blockIdx.y) * 64) + (((int)blockIdx.x) * 8)) + (((int)threadIdx.x) >> 4))] = ((half_t)accum[0]);
  }
}
