resource_usage {}
primary <bound method JITKernel._primary_resource_usage of <tilelang.jit.kernel.JITKernel object at 0x7bfd88bed400>>
...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(const half_t* __restrict__ activation, half_t* __restrict__ output, const uchar* __restrict__ packed_weight, const float* __restrict__ weight_scale, const uchar* __restrict__ weight_zero) {
  ...
  for (int group = 0; group < 640; ++group) {
    __syncthreads();
    activation_shared[((int)threadIdx.x)] = activation[((group * 128) + ((int)threadIdx.x))];
    scales[0] = weight_scale[(((((int)blockIdx.x) * 10240) + ((((int)threadIdx.x) >> 3) * 640)) + group)];
    zeros[0] = ((float)weight_zero[(((((int)blockIdx.x) * 10240) + ((((int)threadIdx.x) >> 3) * 640)) + group)]);
    *(uint2*)(packed_weight_local_cast + 0) = *(uint2*)(packed_weight + ((((((int)blockIdx.x) * 655360) + ((((int)threadIdx.x) >> 3) * 40960)) + (group * 64)) + ((((int)threadIdx.x) & 7) * 8)));
    for (int i = 0; i < 2; ++i) {
      int4 __1;
