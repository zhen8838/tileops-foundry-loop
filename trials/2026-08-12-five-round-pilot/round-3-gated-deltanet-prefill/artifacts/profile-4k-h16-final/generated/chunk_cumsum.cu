...
extern "C" __global__ void __launch_bounds__(128, 1) chunk_cumsum_bthd_kernel_kernel(const bfloat16_t* __restrict__ g, bfloat16_t* __restrict__ out) {
  ...
  if (((int)threadIdx.x) < 16) {
    for (int i = 0; i < 64; ++i) {
      acc_s[((int)threadIdx.x)] = (acc_s[((int)threadIdx.x)] + ((float)g[(((((int)blockIdx.y) * 1024) + (i * 16)) + ((int)threadIdx.x))]));
      out[(((((int)blockIdx.y) * 1024) + (i * 16)) + ((int)threadIdx.x))] = ((bfloat16_t)acc_s[((int)threadIdx.x)]);
    }
  }
}
