"""Profile the full GDN prefill manifest through the interleaved timing API."""
...
class PartitionMode:
    ...
    def __call__(self, *inputs):
        os.environ["TILEOPS_GDN_PREFILL_FORCE_PARTITION"] = (
    ...
    with args.output.open("a", buffering=1) as output_file:
        for seq_len, heads in SHAPES:
            for dtype_name in ("float16", "bfloat16"):
                ...
                candidate_op = GatedDeltaNetPrefillFwdOp(
                    chunk_size=64, layout="bthd"
                )
                ...
                for name in ("candidate", "incumbent"):
                    ...
                    errors[name] = {
                        "max_o_abs": (
                            actual[0].float() - expected[0].float()
                        ).abs().max().item(),
                        "max_state_abs": (
                    ...
                    torch.testing.assert_close(
                        actual[0], expected[0], atol=atol, rtol=rtol
                    )
                    torch.testing.assert_close(
                        actual[1], expected[1], atol=atol, rtol=rtol
                    )
                    del actual
                ...
                route = {
                    ...
                    "incumbent_partition": _prefill_should_partition(
                        seq_len,
                        num_chunks,
                        heads,
                        base_max_local_chunks,
                        True,
                    ),
                    "candidate_max_local_chunks": max_local_chunks,
                    "incumbent_max_local_chunks": base_max_local_chunks,
                }

                benchmark = ManifestBenchmark(
                    "GatedDeltaNetPrefillFwdOp", candidate_op, workload
                )
                timings = {name: [] for name in functors}
                for _ in range(args.trials):
                    results = benchmark.compare(functors, *inputs)
                ...
                del expected, inputs, candidate_op, base_op, functors, benchmark
